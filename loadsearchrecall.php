...
#echo "<p>".$selected ."</p>";
	if($branch == "Recall")
	{
		$sql= "SELECT *, DATE_FORMAT(start, '%h:%i %p' ) as timein_f,  DATE_FORMAT(end, '%h:%i %p' ) as timeout_f, DATE_FORMAT(start, '%b %d, %Y' ) as date_f, DATE_FORMAT(end, '%b %d, %Y' ) as dateend_f FROM comment WHERE (title LIKE '%".$txtSearchDesc."%')  ORDER BY start DESC";
